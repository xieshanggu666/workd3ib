...
- **多灾点资源统筹**：按灾情等级、需求缺口和运输时长自动生成跨基地分配方案；支持人工调整（改基地/改数量/删项）后批量派发；提交时统一校验并锁定库存，库存冲突可一键自动重新分配；派发后联动更新事件状态、运输路线与大屏统计
...
└── src/
    ...
    ├── store/
    │   └── command.js    # Pinia 全局状态、派发与流转逻辑、统筹分配算法
    └── components/
        ...
        ├── CoordinationPanel.vue # 多灾点统筹方案（生成/调整/校验/批量派发）
